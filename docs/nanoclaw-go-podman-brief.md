...
- A generic **base prompt** (`container/CLAUDE.md`) is baked into the runner image at `/app/CLAUDE.md`: identity, safety, the cross-cutting invariants (do-the-deliverable-not-the-narration, honest reporting, timestamp discipline, conciseness), and a mode router. It mentions no vault and no librarian.
