...
> **What the two-DB-per-session boundary is actually FOR (read this, the original framing was misleading).** The early framing sold this as "the host and container talk ONLY through files, no IPC, no sockets," presented as a virtue. That framing is no longer accurate: the channel-plugin boundary (`docs/channels-plugin-design.md`) runs a real TCP/Unix socket across the same host<->container line. So "no networking" was never the principle; it was a side effect. The TWO real reasons the SQLite pair earns its keep are:
>
> 1. **Single-writer-per-cross-mount-file.** Two processes writing ONE SQLite file across the podman VM mount corrupt it: virtiofs page caches are not coherent, so the runner's next write can resurrect the host's row and a reply is sent twice. The fix is one writer per file: the HOST owns `inbound.db` (read-write), the RUNNER owns `outbound.db`, and the host opens `outbound.db` READ-ONLY (`mode=ro`) so an accidental host write fails loudly instead of corrupting silently. This is a property of "SQLite over a VM mount", not of files-vs-sockets; a socket sidesteps it entirely (no shared on-disk page cache), which is exactly why the channel boundary uses one.
> 2. **Durability across a container bounce.** The files are a PERSISTENT QUEUE. If the runner crashes mid-reply, the inbound message is still in `inbound.db` and the host respawns the runner to resume; an at-least-once delivery ledger survives the crash. A bare socket is ephemeral, kill the container and in-flight work on the wire is gone. The agent path wants this durability; the channel path does not (a dropped chat line is just gone), which is why they legitimately differ.
> 3. **Containment (the deciding reason the agent path keeps files, NOT a socket).** The file boundary is PULL-ONLY: the untrusted, prompt-injectable agent has NO live channel to the host. It writes a file in its OWN mount; the host reads it read-only on the host's schedule. The contained process cannot connect out, send the host a frame, or probe, its entire cross-boundary reach is "write bytes into my own filesystem." A socket would invert this, the runner would DIAL the host and hold an open, bidirectional pipe from inside the box to the host process, a live channel a compromised/escaped agent can speak on, added to the MOST sensitive path. So "two mechanisms" (files for the agent, socket for channel plugins) is correct DEFENSE-IN-DEPTH BY TRUST LEVEL: the thing we most want air-gapped (the agent) gets the most-isolated boundary; the narrow, explicitly-untrusted channel plugins get the convenient socket. Uniformity is not the goal; containment is. This was evaluated against moving the agent path to a socket (for the complexity win of deleting the cross-mount tax) and DECIDED in favor of files: no complexity saving justifies giving the box a live channel to the host. Revisit only if the threat model changes, and only if the change proves it does not enlarge the box's reach.
>
> The operational rules below all serve reason (1) and are load-bearing AS LONG AS the agent boundary stays on SQLite-over-the-mount. The temptation that violates it: tracking delivery (`pending → sending → delivered`) by writing status back into `outbound.db`. **Do not.** Instead the **host records delivery in a `delivered` table inside `inbound.db`** (the host-owned file) with an idempotent `INSERT OR IGNORE` keyed by the outbound message id, and **never writes `outbound.db`**. The delivery loop reads `outbound.db` (a 'pending' row there is only a *hint*, never authoritative), checks the ledger, sends, then writes the ledger. Three cross-mount rules: (1) `journal_mode=DELETE` (WAL's `-shm` doesn't refresh across the mount); (2) host opens-writes-closes per op (a long-lived handle freezes its page-cache view); (3) exactly one writer per file, enforced at the DRIVER (`mode=ro`), not by convention. Even with all three, virtiofs can still latch a torn snapshot, so the runner detects a corruption streak and exits for the host to respawn it with a fresh mount. A host opening a not-yet-created `outbound.db` read-only must treat its absence as "nothing pending" rather than an error.
>
> **This whole apparatus exists to make SQLite survive a VM mount.** Whether the agent path should instead move to a socket (keeping SQLite purely as a host-side durability ledger) was evaluated and REJECTED, the file boundary stays, for the containment reason in (3) above. The cross-mount rules are FROZEN: this is the complete set; do not add new SQLite-over-mount special cases (a feature that seems to need one is a signal to reconsider the feature, not the boundary).
...
# (a vaultlock/ flock guard was planned here but dropped; see §11.5 — the vault's
#  concurrency protection is the librarian task-lease + git-revert discipline)
...
- A generic **base prompt** (`container/CLAUDE.md`) is baked into the runner image at `/app/CLAUDE.md`: identity, safety, the cross-cutting invariants tiered `must`/`should`/`may` (honest reporting and the containment boundary are `must`; conciseness is `should`; timestamp format is `may`; see `docs/context-and-guardrails.md` §2a), and a mode router. It mentions no vault and no librarian.
...
2. **A write lock (SUPERSEDED, not built).** The original plan was a host-side `flock` on a vault lockfile taken before launching any vault-mutating agent run. This was dropped: a host flock around `podman run` serializes none of the actual writers, since the agent writes to `/vault` from inside a long-lived warm container (not a discrete host-launched run) and the human writes from Obsidian. The shipped protection is instead **prompt discipline in the librarian skill**: task notes carry a `claimed_by` + `lease_until` lease (claim-before-working, heartbeat to extend, stale leases are reclaimable), every mutation appends to an append-only `log.md`, and the vault is a git repo so a bad write is revertible. A scaffolded `internal/vaultlock` package implementing the flock was deleted as never-wired dead code.
