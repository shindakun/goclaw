...
>
> **This whole apparatus exists to make SQLite survive a VM mount.** Now that a socket boundary exists and works, whether the agent path should ALSO move to a socket (keeping SQLite purely as a host-side durability ledger the container never touches) is a live redesign question, see `docs/boundary-redesign.md`.
