...
// The schedule is either a LOCAL wall-clock boundary (atHour 0-23 + atMinute 0-59, fires
// once per periodDays at/after that hour:minute, in the host's local zone, so it does not
// drift with host uptime) OR a pure interval (atHour < 0: fire when now-lastRun >= every).
//
...
func IsDue(periodDays, atHour, atMinute int, every time.Duration, lastRun time.Time, ranBefore bool, now time.Time) bool {
	// No preferred hour: pure interval scheduling.
	if atHour < 0 {
		return !ranBefore || now.Sub(lastRun) >= every
	}

	// Today's atHour:atMinute boundary in the host's local zone.
	boundary := time.Date(now.Year(), now.Month(), now.Day(), atHour, atMinute, 0, 0, now.Location())
